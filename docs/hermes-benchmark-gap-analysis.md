...
Telegram (private or group chat today) ── grammY bot
  ...
  └─ AssistantEngine.reply(chatId, text)
          ...
          ├─ Configured Gemini 3.6 Flash (or another supported model) OR OpenAI gpt-4o
...
| LLM/model routing | `getModel()` in `agent.service.ts`; config in `src/config/env.ts`. | Selects configured Gemini (default 3.6 Flash), or gpt-4o only when OpenAI is selected and a key exists. | Provider abstraction comes from Vercel AI SDK. This is a static selection, not routing or failover: OpenAI selection silently falls back to Gemini when the key is absent; retired Gemini 2.0 environment values are mapped to the current default with a warning; no runtime/provider fallback is implemented. |
